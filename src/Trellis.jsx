...
export default memo(function Trellis() {
  ...
  const pattern = getSelected(state, "topLeft", true).map((object, index) =>
    object.render(state, `trellis-pattern-${index}`, debug ? { stroke: "red", strokeWidth: 2 / scalex } : {}, false, boundRect),
  )
  ...
  const dimDiff =
    trellisFlip.row.val || trellisFlip.col.val || trellisRotate.row.val || trellisRotate.col.val
  ...
  const applyTransformations = (x, y, row, col) => {
    // Skip
    if (
      Math.abs(col) % (trellisSkip.row.every + trellisSkip.row.val) < trellisSkip.row.every &&
      ...
      if (!(col % (trellisOverlap.row.every + 1))) {
        if (trellisOverlap.row.val.x) transformation += `translate(${trellisOverlap.row.val.x}, 0)`
        if (trellisOverlap.row.val.y) transformation += `translate(0, ${trellisOverlap.row.val.y})`
      }
      if (!(row % (trellisOverlap.col.every + 1))) {
        if (trellisOverlap.col.val.x) transformation += `translate(${trellisOverlap.col.val.x}, 0)`
        if (trellisOverlap.col.val.y) transformation += `translate(0, ${trellisOverlap.col.val.y})`
      }
  ...
  function addPattern(transformation) {
    if (transformation) rtn.add(transformation)
  }
