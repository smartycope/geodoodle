    ...
    return (
        debug && (
            <g>
                ...
                {debugDrawPoints &&
                    Object.entries(debugDrawPoints).map(([name, spec]) => (
                        <DebugPoint key={name} name={name} {...spec} />
                    ))}
                {/* Draw intersections */}
...
export const MirrorMetaLines = () => {
    ...
    const { x: cursorLinex, y: cursorLiney } =
        curLinePos === null ? { x: null, y: null } : curLinePos.asViewport(state);
