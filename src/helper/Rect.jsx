...
export default class Rect {
  ...
  asSvg(state = undefined, inflate = true) {
    if (!state && inflate) throw new Error("Cannot deflate rect without scalex and scaley")
  ...
  get bottomLeft() {
    return new Point(this.topLeft._x, this.bottomRight._y)
  }
  get topRight() {
    return new Point(this.bottomRight._x, this.topLeft._y)
  }
  ...
  // Note that this rounds to the nearest dot
  get centerOffset() {
    return this.bottomRight.sub(this.topLeft).div(2).round().asDist()
  }

  // width and height, together as a Dist
  get wh() {
    return new Dist(this.bottomRight._x - this.topLeft._x, this.bottomRight._y - this.topLeft._y)
  }

  render(state, props = {}) {
    const { left, top, width, height } = this.asViewport(state)
    return <rect x={left} y={top} width={width} height={height} {...props} />
  }

  grow(amount) {
    return new Rect(this.topLeft.sub(new Dist(amount, amount)), this.bottomRight.add(new Dist(amount, amount)), false)
  }
  shrink(amount) {
  ...
  onCorner(point) {
    return (
      (point._x === this.topLeft._x && point._y === this.topLeft._y) ||
      (point._x === this.bottomRight._x && point._y === this.bottomRight._y)
    )
  }
}
