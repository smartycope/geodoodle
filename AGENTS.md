...
- `src/Trellis.jsx` separately renders the repeated background pattern. The unchanged Paper-owned selection is conceptual tile `(0, 0)` and is never duplicated by Trellis; its row and column anchor every skip/offset/flip/rotation cadence. Trellis derives a finite lattice range from all four inverse-transformed viewport corners, then culls transformed lines and polygons before emitting SVG groups. Repeat controls use row/column `{ every, val }` structures.
